    ...
    def generate_cluster_insights(self, cluster_results):
        ...
        try:
            ...
            if cluster_sizes:
                ...
                insights.append(f"最大的聚类组(聚类{largest_cluster})包含{largest_size}个项目，占总数的{largest_percent:.1f}%")
                ...
                if 'cluster_analysis' in cluster_results:
                    ...
                    for cluster_id, cluster_info in analysis.items():
                        ...
                        if 'age_group' in cluster_info:
                            insights.append(f"聚类{cluster_id}的人群画像: 以{cluster_info.get('age_group', '')}为主，{cluster_info.get('gender_pref', '')}，兴趣多样性{cluster_info.get('interest_diversity', '')}")
                        else:
        ...
        try:
            ...
            if "visualization" in result:
                ...
                try:
                    ...
                    # 创建保存路径
                    plot_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'data', 'clusters')
