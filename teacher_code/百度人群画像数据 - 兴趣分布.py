...
print("*****  开始采集百度人群画像-兴趣爱好二级数据   *****")
for word in words: # 搜索关键词
    url3 = "https://index.baidu.com/api/SocialApi/interest?wordlist[]={}&typeid={}".format(word,10000000)  # 设置兴趣分布url
    #encode_data = quote(word)  # encode处理
    print("| 正在读取:{}的人群画像-兴趣爱好-汽车二级数据.........".format(word))
    res_index = requests.get(url3,headers=headers)
    time.sleep(3)
    json_index = res_index.json()   #转json格式
    #print(json_index)  # 辅助测试获取数据格式
    if json_index['data']!="":
        list_index_interest =json_index['data']['result'][0]['interest'] # 兴趣分布数据
        # print(data_list)
        # 数据格式为{}
        ...
        # 构建excel格式数据[{},{},{}],每{}为一行数据
        data_all = pd.DataFrame.from_dict(list_index_interest)  # 将性别字典列表转pandas格式DF
        filename = "d:\\人群画像\\兴趣爱好\\" + str(word) + "兴趣爱好二级数据" + ".xlsx"
        print("| 正在写入文件->{}............".format(filename))
        data_all.to_excel(filename, sheet_name="兴趣爱好", index=False)  # 写入excel文件
        time.sleep(2)
